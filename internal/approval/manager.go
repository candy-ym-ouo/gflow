...
func (m *Manager) Create(i *model.WorkflowInstance, n model.Node) (*model.ApprovalTask, error) {
	...
	t := &model.ApprovalTask{ID: fmt.Sprintf("task-%d", time.Now().UnixNano()), InstanceID: i.ID, NodeID: n.ID, Mode: fmt.Sprint(n.Config["mode"]), Status: "PENDING", Assignees: a, PassRatio: 1}
	if t.Mode == "" {
		t.Mode = "single"
	}
